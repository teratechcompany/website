...
export async function GET(req: Request) {
  ...
  return NextResponse.json({
    results:[
      ...blog.map   ((r: Record<string, unknown>) => ({ ...r, _type:'blog'    })),
      ...events.map ((r: Record<string, unknown>) => ({ ...r, _type:'event'   })),
      ...alumni.map ((r: Record<string, unknown>) => ({ ...r, _type:'alumni'  })),
      ...careers.map((r: Record<string, unknown>) => ({ ...r, _type:'career'  })),
    ]
  })
}
